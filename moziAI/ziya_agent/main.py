...
from ziya_agent.env import EnvZiya as Environment
from ziya_agent import etc
from ziya_agent.agent import Agent
...
def main():
    ...
    #创建环境对象
    env = Environment(etc.SERVER_IP, etc.SERVER_PORT, etc.SCENARIO_NAME, etc.SIMULATE_COMPRESSION, etc.SYNCHRONOUS, etc.DURATION_INTERVAL, etc.SERVER_PLAT)
    #启动环境
    env.start()
    #创建智能体对象
    agent = Agent(env)
