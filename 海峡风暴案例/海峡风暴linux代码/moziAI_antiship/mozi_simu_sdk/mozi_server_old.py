...
class MoziServer:
    ...
    def __init__(self, server_ip, server_port, platform, scenario_name='', compression=5, synchronous=True):
        ...
        self.is_connected = self.connect_grpc_server() # 这应该时初始化GRPC客户端
    ...
    def load_scenario_in_linux(self, path, model):
        ...
        # return self.send_and_recv("Hs_ScenEdit_LoadScenario('{}', {})".format(path, model)) # true：以推演模式加载,false：以编辑模式
    ...
    def init_situation(self, scenario):
        ...
        初始化态势
        """
        # bInitSuccess:bool = scenario.situation.init_situation(self, scenario)
        bInitSuccess = scenario.situation.init_situation(self, scenario)
        return bInitSuccess
