...
from mozi_ai_sdk.test.dppo.agents.utils_tf import explained_variance
from mozi_ai_sdk.test.dppo.utils.utils import tprint

from mozi_ai_sdk.test.dppo.envs.env import Environment
from mozi_ai_sdk.test.dppo.envs import etc
from mozi_ai_sdk.test.dppo.envs.observations import Features
from mozi_ai_sdk.test.dppo.envs.tasks import Task
...
class Model(object):
    def __init__(self,
        ...
        def train(lr,
            ...
            return sess.run([
                pg_loss, vf_loss, entropy, approxkl, clipfrac, summary_op,
                _train
            ], td_map)[:-1]
        ...
        # TODO
        self.writer = tf.summary.FileWriter("./tfevents_ppo/",
    ...
    def _nstep_rollout(self, num):
        ...
        for _ in range(self._unroll_length):
            action, value, self._state, neglogpac = self._model.step(
                ...
                self._state, np.expand_dims(self._done, 0))
        ...
        if isinstance(self._obs, tuple):
            mb_obs = tuple(
                np.asarray(obs, dtype=self._obs[0].dtype)
        ...
        return (mb_obs, mb_returns, mb_dones, mb_actions, mb_values,
                mb_neglogpacs, mb_states, episode_infos)
...
class PPOLearner(object):
    ...
    def run(self):
        ...
        while True:
            ...
            loss_tuple = self._model.train(lr_now, clip_range_now, obs,
                                           returns, dones, actions, values,
                                           neglogpacs, states)
            ...
            if updates % self._print_interval == 0:
                ...
                avg_reward = safemean(
                    [info['r'] for info in self._episode_infos])
                tprint("Update: %d	Train-fps: %.1f	Rollout-fps: %.1f	"
                       ...
                       (updates, train_fps, rollout_fps, var, avg_reward,
    ...
    def _prepare_batch(self, data_queue, batch_queue, batch_size):
        while True:
            ...
            obs, returns, dones, actions, values, neglogpacs, states = zip(
                *batch)
...
class PPOSelfplayActor(object):
    ...
    def _nstep_rollout(self):
        ...
        for _ in range(self._unroll_length):
            ...
            oppo_action, _, self._oppo_state, _ = self._oppo_model.step(
                transform_tuple(self._oppo_obs,
                                lambda x: np.expand_dims(x, 0)),
                self._oppo_state, np.expand_dims(self._done, 0))
            mb_obs.append(transform_tuple(self._obs, lambda x: x.copy()))
            mb_actions.append(action[0])
            mb_values.append(value[0])
            mb_neglogpacs.append(neglogpac[0])
            mb_dones.append(self._done)
            (self._obs,
             self._oppo_obs), reward, self._done, info = self._env.step(
    ...
    def _update_opponent(self):
        if (random.uniform(0, 1.0) < self._prob_latest_opponent
            ...
            tprint(
                "Opponent updated with the previous model. %d models cached." %
