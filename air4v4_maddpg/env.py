...
def get_target_point():
    ...
    获取目标点
    '''
    lat2 = etc.task_end_point["latitude"]  # 坦克终点
...
def _get_reward_value(task_heading, current_heading, distance):
    ...
    获取奖励值
    '''
    angel = abs(task_heading - current_heading)  # task_heading是啥？是角度吗？
    cos_value = cos(radians(angel))
    # 定义当前位置和目标位置的cos差的函数为奖惩值
    if cos_value >= 0:
...
def get_distance_reward(lat, lon, last_heading, heading_change):
    '''
    获取距离奖励值
    get_degree(latA, lonA, latB, lonB):
        """
        获得朝向与正北方向的夹角
        :param latA: A点的纬度
        :param lonA: A点的经度
...
class Air4v4Env(base_env.BaseEnvironment):
    '''
    伊拉克想定环境类
    '''
    def __init__(self,
                 ...
                 server_plat="windows"):
        super(Air4v4Env,
              self).__init__(IP, AIPort, server_plat, scenario_name,
        ...
        self.red_side_name = "红"
        self.blue_side_name = "蓝"
    ...
    def _check_target_exist(self):
        ...
        for key in ret:
            ret = self.scenario.unit_is_alive(key)  # key是目标的名字或编号
            if not ret:
        ...
        for guid in airs:
            ...
            if distance < etc.target_radius:
                if self._check_is_find_target():
                    ...
                    #print("target_guid:%s" % target_guid)
    ...
    def reset(self):
        ...
        '''
        # 报错：
        # super(Air4v4Env, self).reset()
        # TypeError: reset() missing 1 required positional argument: 'app_mode'
        #######################
        # app_mode:
        ...
        #######################
        super(Air4v4Env, self).reset(app_mode = 1)
        self.scenario.set_cur_side_and_dir_view("蓝", "false")
        # 设置当前推演方及是否显示导演视图
        self._construct_side_entity()
        self._init_unit_list()
        return self._get_timesteps(None)
