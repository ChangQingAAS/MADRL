...
class BaseEnvironment:
    ...
    def __init__(self, IP, AIPort, platform, scenario_name, simulate_compression, duration_interval):
        ...
        # self.scenario = self.mozi_server.scenario
    ...
    def reset(self):
        ...
        bInitSucess: bool = self.mozi_server.init_situation(self.scenario)
        # if not bInitSucess:
        #     # 弹出对话框，提示失败
        self.mozi_server.set_simulate_compression(self.simulate_compression)
        self.mozi_server.run_simulate() # 先别启动运行。

        # self.steps = 0
        # self.scenario = self.mozi_server.load_scenario()
        # self.mozi_server.set_simulate_compression(self.simulate_compression)
        # self.mozi_server.init_situation(self.scenario, self.server_ip)
    ...
    def connect_mozi_server(self):
        ...
        self.mozi_server = MoziServer(
            ...
            self.synchronous)
        #self.mozi_server = self.mozi_server
        return True
