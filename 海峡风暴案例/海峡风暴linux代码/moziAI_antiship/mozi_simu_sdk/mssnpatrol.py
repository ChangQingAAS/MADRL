...
class CPatrolMission(CMission):
    ...
    def add_patrol_zone(self, side_name, patrol_name, zone_list):
        """
        添加巡逻区
        :param side_name:
        :param patrol_name:
        ...
        :return:
        """
        cmd = "ScenEdit_SetMission('%s', '%s', {PatrolZone = {'%s','%s','%s','%s'}})" % (
            side_name, patrol_name, zone_list[0], zone_list[1], zone_list[2], zone_list[3])
    ...
    def set_one_third_rule(self, side_name, is_one_third):
        """
        设置任务是否遵循1/3原则
        ...
        时间：4/10/20
        """
        return super().set_one_third_rule(side_name, is_one_third)

    def set_opa_check(self, ischeckOPA):
        """
    ...
    def set_wwr_check(self, ischeckWWR):
        """
    ...
    def set_flight_size(self, enum_flight_size):
        """
        设置任务编队规模
        ...
        """
        cmd = "ScenEdit_SetMission('" + str(self.m_Side) + "', '" + str(
            self.strGuid) + "', { flightSize = " + str(enum_flight_size) + "})"
        # self.strGuid) + "', { flightSize = " + str(enum_flight_size.value) + "})"
        self.mozi_server.throw_into_pool(cmd)
        return self.mozi_server.send_and_recv(cmd)

    def set_flight_size_check(self, useFlightSize):
        """
        是否飞机数低于编队规模不允许起飞
        ...
        时间：4/10/20
        """
        cmd = 'ScenEdit_SetMission("' + self.m_Side + '","' + self.strName + '", {''useFlightSize =' + str(
            useFlightSize).lower() + '})'
        self.mozi_server.throw_into_pool(cmd)
        return self.mozi_server.send_and_recv(cmd)
