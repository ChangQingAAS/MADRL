...
class PPOActor(object):
  ...
  def __init__(self, env, policy, unroll_length, gamma, lam, queue_size=1,
    ...
    self._state_code = deque(maxlen=1)  # 'restart'/'sucessloadscen'
    self._state_push_thread = Thread(target=self._state_push_data, args=(self._zmq_context, self._state_code, '5704'))
    self._state_push_thread.start()

    self._state_code_2 = deque(maxlen=1)  # 'restart'/'sucessloadscen'
    self._state_2_push_thread = Thread(target=self._state_push_data, args=(self._zmq_context, self._state_code_2, '5705'))
    self._state_2_push_thread.start()

  def _state_push_data(self, zmq_context, state_code, port):
      sender = zmq_context.socket(zmq.PUSH)
      sender.setsockopt(zmq.SNDHWM, 1)
      sender.setsockopt(zmq.RCVHWM, 1)
      sender.connect("tcp://%s:%s" % ('localhost', port))
      while True:
        if len(state_code) > 0:
            data = state_code.pop()
  ...
  def _nstep_rollout(self):
    ...
    for _ in range(self._unroll_length):
      ...
      if self._done:
        # TODO 向slave发送一局结束状态
        self._state_code.append('restart')
        # TODO 重新加载想定，想定加载成功会向slave发送想定加载成功的标志；
        self._obs = self._env.reset()
        self._state_code.append('sucessloadscen')
        self._state = self._model.initial_state
...
class PPOSelfplayActor(object):
  ...
  def __init__(self, env, policy, unroll_length, gamma, lam, model_cache_size,
    ...
    self._obs, self._oppo_obs, _, _, _ = self._env.step({'红方': 0, '蓝方': 0})
    self._state = self._model.initial_state
