...
class CSide:
    ...
    def get_patrol_missions(self):
        ...
        # return {k: v for k, v in self.mssnpatrol_dic.items() if v.m_Side == self.strGuid}
    ...
    def add_mission_patrol(self, name, patrol_type_num, zone_points):
        ...
        detail = ["{type='", patrol_type, "', Zone={'%s'}}"]
        points = []
    ...
    def delete_mission(self, mission_name):
        ...
        for k, v in list(self.missions.items()):
            if v.strName == mission_name:
                del self.missions[k]
    ...
    def add_reference_point(self, name, lat, lon):
        ...
        时间：4/24/20
        """
        cmd = "ScenEdit_AddReferencePoint({side='%s', name='%s', lat=%s, lon=%s})" % (self.strName, name, lat, lon)
        rslt = self.mozi_server.send_and_recv(cmd)
        pnt = None
        if rslt == 'lua执行成功':
            self.mozi_server.throw_into_pool(cmd)
    ...
    # def set_reference_point(self, name, new_coord):
    def set_reference_point(self, name, lat, lon):
        """
        设置参考点的位置
        :param : tuple, 新的经纬度位置 (lat, lon)
        :return:
        """
    ...
    def delete_reference_point(self, pnts):
        """
        删除参考点
        :return:
        """
        lua_str = ""
        for pnt in pnts:
            lua_str += 'ScenEdit_DeleteReferencePoint({side="%s",name="%s"})' % (self.strName, pnt)
    ...
    #     :return:
    #     """
    #     return self.mozi_server.send_and_recv("Hs_AddPlanWayToMission('{}',{},'{}')".
    ...
    def add_plan_way_to_mission(self, strMissionNameOrID, Type, strWayNameOrID):
        ...
        """
        return self.mozi_server.send_and_recv(
            "Hs_AddPlanWayToMission({MISSIONNAME='%s',MISSIONWAYTYPE=%d,PLANWAYNAME='%s'})" % (
                strMissionNameOrID, Type, strWayNameOrID))
