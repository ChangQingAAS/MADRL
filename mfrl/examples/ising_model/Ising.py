...
class Scenario():
    def _calc_mask(self, agent, shape_size):
        ...
        if agent.view_sight == -1:
            ...
            # observe neighbours
            delta = list(
                range(-int(agent.view_sight),
                      int(agent.view_sight) + 1, 1))
            delta.remove(
                0)  # agent itself is not counted as neighbour of itself
            for dt in delta:
            ...
            if agent.state.p_pos[0] < agent.view_sight:
                tar = shape_size - (np.array(
                    range(0, int(agent.view_sight - agent.state.p_pos[0]), 1))
                                    + 1)
            ...
            if agent.state.p_pos[1] < agent.view_sight:
                tar = shape_size - (np.array(
                    range(0, int(agent.view_sight - agent.state.p_pos[1]), 1))
                                    + 1)
            ...
            if agent.state.p_pos[0] >= shape_size - agent.view_sight:
                tar = np.array(
                    range(
                        0,
                        int(agent.view_sight -
                            (shape_size - 1 - agent.state.p_pos[0])), 1))
                tar = tar * shape_size + agent.state.p_pos[1]
                agent.spin_mask[tar] = [1] * len(tar)
            ...
            if agent.state.p_pos[1] >= shape_size - agent.view_sight:
                tar = np.array(
                    range(
                        0,
                        int(agent.view_sight -
                            (shape_size - 1 - agent.state.p_pos[1])), 1))
                tar = agent.state.p_pos[0] * shape_size + tar
                agent.spin_mask[tar] = [1] * len(tar)
    ...
    def make_world(self, num_agents=100, agent_view=1):
        ...
        world.shape_size = int(
            np.ceil(np.power(num_agents, 1.0 / world.dim_pos)))
        ...
        world.agents = [
            IsingAgent(view_sight=world.agent_view_sight)
    ...
    def reward(self, agent, world):
        ...
        mask_display = agent.spin_mask.reshape(
            (int(np.sqrt(world.n_agents)), -1))
