...
class CMission:
    ...
    def is_active(self, is_active=''):
        ...
        lua = "print(ScenEdit_SetMission('%s','%s',{isactive='%s'}))" % (self.m_Side, self.strName, str_set)
    ...
    def set_one_third_rule(self,side_name, is_one_third):
        ...
        cmd = 'ScenEdit_SetMission("%s","%s", {oneThirdRule=%s})' % \
              (side_name, self.strName, str(is_one_third).lower())
        self.mozi_server.throw_into_pool(cmd)
        return self.mozi_server.send_and_recv(cmd)

    def main_position(self, side_name, pos_num):
        cmd = 'ScenEdit_SetMission("%s","%s", {SupportMaintainUN=%s})' % \
              (side_name, self.strName, pos_num)
        return self.mozi_server.send_and_recv(cmd)
