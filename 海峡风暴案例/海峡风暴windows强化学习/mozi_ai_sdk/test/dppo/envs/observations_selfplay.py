...
class Features(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.sideName == '红方':
            ...
            obs = self._features()
            info = {}
            return scenario, (obs, mask), reward, done, info
        else:
            scenario, mask, red_obs_mask, reward, done, info = self._env.step(action)
            self._update(scenario)
            obs = self._features()
            return red_obs_mask, (obs, mask), reward, done, info
    ...
    def reset(self):
        if self.sideName == '红方':
            ...
            self._update(scenario)
            obs = self._features()
            return scenario, (obs, mask)
        else:
            scenario, mask, red_obs_mask = self._env.reset()
            self._update(scenario)
            obs = self._features()
            return red_obs_mask, (obs, mask)
