...
class CDoctrine:
    ...
    def _classify_owner(self, owner_class_name):
        """
        功能：对条令所属对象进行分类
    ...
    def avoid_being_searched_for_submarine(self, decision):
        """
        功能：决定是否规避搜索。
        参数：decision: {str: 'No'('0')-否, 'YesExceptSelfDefence'('1')除非自卫均是,'YesAlways'('2')-总是}
        返回：'lua执行成功' 或 '脚本执行出错'
        作者：aie
        单位：北京华戍防务技术有限公司
        时间：4/3/20
    ...
    def dive_on_threat(self, decision):
        ...
        参数：decision: {str: 'Yes'('0')-在敌潜望镜或对面搜索雷达侦察时下潜,
                             'YesESMOnly'('1')-在敌电子侦察措施侦察或目标接近时下潜,
                             'YesShips20nmAircraft30nm'('2')-在20海里内有敌舰或30海里内有敌机时下潜,
                             'No'('3')-否}
        返回：'lua执行成功' 或 '脚本执行出错'
        作者：aie
        单位：北京华戍防务技术有限公司
    ...
    def set_recharging_condition_on_patrol(self, recharging_condition):
        """
        功能：设置出航或阵位再充电条件。
        参数：recharging_condition: {string: 'RechargeEmpty'('0')-电量用完再充,
                                            'Recharge10Percent'('10')-电量剩下10%时再充,
                                            'Recharge20Percent'('20')-电量剩下20%时再充,
                                            'Recharge30Percent'('30')-电量剩下30%时再充,
                                            'Recharge40Percent'('40')-电量剩下40%时再充,
                                            'Recharge50Percent'('50')-电量剩下50%时再充,
                                            'Recharge60Percent'('60')-电量剩下60%时再充,
                                            'Recharge70Percent'('70')-电量剩下70%时再充,
                                            'Recharge80Percent'('80')-电量剩下80%时再充,
                                            'Recharge90_Percent'('90')-电量剩下90%时再充}
        返回：'lua执行成功' 或 '脚本执行出错'
        作者：aie
        单位：北京华戍防务技术有限公司
        时间：4/3/20
    ...
    def use_aip(self, decision):
        ...
        参数：decision: {str: 'No'('0')-否, 'YesAttackOnly'('1')-在进攻或防御时使用, 'YesAlways'('2')-总是}
        返回：'lua执行成功' 或 '脚本执行出错'
        作者：aie
        单位：北京华戍防务技术有限公司
        时间：4/3/20
    ...
    def use_dipping_sonar(self, decision):
        ...
        owner_type = self._classify_owner(owner.ClassName)
        if owner_type == 'Side':
            cmd = "ScenEdit_SetDoctrine({side ='%s' }, {dippint_sonar ='%s'})" % (
                owner.strGuid, decision)
        elif owner_type == 'Mission':
            cmd = "ScenEdit_SetDoctrine({side ='%s', mission = '%s'}, {dippint_sonar ='%s'})" % (
                owner.m_Side, owner.strGuid, decision)
        else:
            cmd = "ScenEdit_SetDoctrine({guid ='%s' }, {dippint_sonar ='%s'})" % (
                owner.strGuid, decision)
        self.mozi_server.throw_into_pool(cmd)
        return self.mozi_server.send_and_recv(cmd)
    ...
    def set_weapon_release_authority(self, weapon_dbid, target_type, quantity_salvo=2, shooter_salvo=1,
        ...
        """
        功能：设置条令的武器使用规则
        参数：weapon_dbid: {str: 武器的数据库ID,就是点击AIM120武器单元弹出来的那个武器介绍的编号：51}
