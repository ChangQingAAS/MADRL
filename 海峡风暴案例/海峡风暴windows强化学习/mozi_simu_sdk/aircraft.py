...
class CAircraft(CActiveUnit):
        ...
        for mount_obj in self.mounts.values():
            if (mount_obj.strWeaponFireState == "就绪" or "秒" in mount_obj.strWeaponFireState) \
                ...
                for w_record in mount_weapons:
                    # 修改变量名，以及字典的键的名称 董卓 2020年9月25日15:13:45
                    w_dbguid = w_record['wpn_dbguid']
                    # 获取数据，服务端发的数据直接解析就可以 董卓 2020年9月25日15:12:37
                    # if db.check_weapon_attack(w_dbid):
                    if w_dbguid in info:
                        info[w_dbguid] += w_record['wpn_current']
                    else:
                        info[w_dbguid] = w_record['wpn_current']
    ...
    def assign_unitlist_to_mission_escort(self, mission_guid):
        ...
        类别：推演所用函数
        将单元分配为某打击任务的护航任务
        :param mission_guid: 任务guid
        """
        # 修改参数为任务的guid，由于名称会重复，guid是不会有重复的 董卓 2020年9月26日17:11:45
        return super().assign_unitlist_to_mission_escort(mission_guid)
    ...
    def set_airborne_time(self, hour, minute, scond):
        ...
        scond 秒
        """
        # 修改Lua,参数是字符串类型的 董卓 2020年9月27日10:25:24
    ...
    def set_fuel_qty(self, remainingFuel):
        ...
        函数功能：通过设置飞机的剩余油量来。
        参数说明：
        1）UnitID：字符串。单元 GUID；
        2）RemainingFuel：字符串。剩余测量。
        """
        # 修改Lua接口，使用单元的GUID 董卓 2020年9月26日17:53:05
        lua = " Hs_SetFuelQty('%s','%s')" % (self.strGuid, remainingFuel)
        self.mozi_server.send_and_recv(lua)
    ...
    def abort_launch(self):
        ...
        @return:
        """
        # 修改Lua,之前写的Lua参数不正确 董卓 2020年9月27日10:44:15
        return self.mozi_server.send_and_recv("Hs_ScenEdit_AirOpsAbortLaunch({{guid = '{}'}})".format(self.strGuid))

    def refuel(self, tanker_or_mission_guid, air_refuel_by_mission = False ):
        """
        ...
        """
        # 受油对象不能是空，可以使任务，也可以是单元 董卓 2020年9月27日16:49:25
        if tanker_or_mission_guid == '':
            return "参数不正确"
            # lua = "Hs_ScenEdit_AirRefuel({guid='%s'})" % guid
        else:
            lua = ""
            # 当使用任务中的加油时
            if air_refuel_by_mission:
                lua = "Hs_ScenEdit_AirRefuel({guid = '%s', MISSION_GUID = '%s'})" % (self.strGuid, tanker_or_mission_guid)
            # 否则选择加油机
            else:
                lua = "Hs_ScenEdit_AirRefuel({guid = '%s', tanker_guid = '%s'})" % (self.strGuid, tanker_or_mission_guid)
