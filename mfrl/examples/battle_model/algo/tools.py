...
class MetaBuffer(object):
    ...
    def append(self, value):
        ...
        # 如果value的长度 + 当前指针 > max_len
        # 先把data【】中指针后为空的部分加上（此时data已满
        # 再把剩余的value[]覆盖到data【】的前面的部分
...
class EpisodesBuffer(Buffer):
    """重放缓冲区，用于存储 a whole episode for all agents
       one entry for one agent
    """
    def __init__(self, use_mean=False):
        super().__init__()
        self.buffer = {}
        # 不知道这里为啥用mean，而不是mf，这个变量表示，重放经验时需要多加一个邻居平均动作
...
class MemoryGroup(object):
    def __init__(self, obs_shape, feat_shape, act_n, max_len, batch_size, sub_len, use_mean=False):
        ...
        self.obs_shape = obs_shape # shape是啥？ 还是观测？
        self.feat_shape = feat_shape
        self.sub_len = sub_len # sub是啥？
        self.use_mean = use_mean
        self.act_n = act_n # 有的变量是n_act ，
        ...
        self.rewards = MetaBuffer((), max_len)
        self.terminals = MetaBuffer((), max_len, dtype='bool')
        self.masks = MetaBuffer((), max_len, dtype='bool') # masks掩码？
        if use_mean:
            self.prob = MetaBuffer((act_n,), max_len)
        self._new_add = 0
    ...
    def _flush(self, **kwargs):
        ...
        # where()返回 等于 True, False, True对应的位置
    ...
    def register(self, name_list):
        ...
        with self.gra.as_default():
            for name in name_list:
                ...
                # 写一个标量摘要 tf.summary.scalar(name, data, step=None, description=None )
