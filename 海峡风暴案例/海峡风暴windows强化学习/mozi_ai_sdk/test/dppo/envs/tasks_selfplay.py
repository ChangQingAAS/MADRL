...
class Task(object):
    ...
    def step(self, action):
        if self.sideName == '红方':
            mission_unit = self._assign_available_unit(action['红方'])
            self._actions[action['红方']].function(mission_unit)
            print('action:', action)
            ...
            mission_unit = self._assign_available_unit(action['蓝方'])
            self._actions[action['蓝方']].function(mission_unit)
            print('action:', action)
            scenario, red_obs_mask, reward, done, info = self._env.step(action)
            self._update(scenario)
            mask = self._get_valid_action_mask()
            return scenario, mask, red_obs_mask, reward, done, info

    def reset(self):
        if self.sideName == '红方':
            scenario = self._env.reset()
            self._update(scenario)
            mask = self._get_valid_action_mask()
            return scenario, mask
        else:
            scenario, red_obs_mask = self._env.reset()
            self._update(scenario)
            mask = self._get_valid_action_mask()
            return scenario, mask, red_obs_mask
    ...
    def _SetTaskParam(self, mission, kwargs):
        ...
        # 设置编队规模
        mission.m_Side = self.sideName
        mission.strName = kwargs['missionName']
