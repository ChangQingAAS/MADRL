...
def out_put_log(level, content, log_path="", log_key="log", stack_layer=2, ):
    func_name = sys._getframe(stack_layer).f_code.co_name # 当前函数名
    file_name = sys._getframe(stack_layer).f_code.co_filename # 当前文件名，可以通过__file__获得
    line = sys._getframe(stack_layer).f_lineno # 当前行号
