...
class CSide:
    ...
    def add_unit(self, unit_type, name, dbid, latitude, longitude, heading):
        """
        功能：添加单元
        参数：dbid格式为27，而不是 hsfw-dataaircraft-000000000027，飞机的挂载方案数据格式也是如此：30，而不是hsfw-dataloadout-0000000000030
        返回：
        作者：aie
        单位：北京华戍防务技术有限公司
        时间：4/26/20
    ...
    def add_group(self, unitGuidList):
        ...
        2）返回参数：编队信息组成的表对象。
        用法：
        scenEdit_AddGroup({'613f00e1-4fd9-4715-a672-7ec5c22486cb','431337a9-987e-46b6-8cb8-2f92b9b80335','0bc31a3c-096a-4b8e-a23d-46f7ba3b06b3'})
        """
    ...
    def set_side_options(self, awareness, proficiency, isAIOnly, isCollRespons):
        ...
        """
        return self.mozi_server.send_and_recv(
            "ScenEdit_SetSideOptions({side=%s,awareness=%s,proficiency=%s,isAIOnly=%s,isCollRespons=%s})"%(self.strName,
                                                                    awareness, proficiency, isAIOnly, isCollRespons))
    ...
    def delete_unit(self, unit_name):
        ...
        作者：赵俊义
        日期：2020-3-9
        修改：2021-6-25 保留{}，外层需要再嵌套一个{}
        函数类别：编辑所用的函数
        功能说明：删除当前推演方中指定单元
        @param unit_name: 单元名
        @return:
        """
        return self.mozi_server.send_and_recv("ScenEdit_DeleteUnit({{name='{}'}})".format(unit_name))
    ...
    def add_plan_way_to_mission(self, strMissionNameOrID, Type, strWayNameOrID):
        ...
        """
        return self.mozi_server.send_and_recv("Hs_AddPlanWayToMission('%s',%d,'%s')"%(strMissionNameOrID,Type, strWayNameOrID))
