...
def situationAwareness(scenario):
    ...
    targets = {
        ...
        if ('FFG530' in v.strName) or ('DDG173' in v.strName)
    }
    if len(targets) == 0:
        return False
    missionUnits = {
        ...
        if v.strAirOpsConditionString in [0, 19, 20, 21]
    }
    for unitGuid, unit in missionUnits.items():
        ...
        for k, v in targets.items():
            if v.m_IdentificationStatus >= 3:
                disKilo = get_two_point_distance(unitPos['Longitude'],
                                                 unitPos['Latitude'],
                                                 v.dLongitude, v.dLatitude)
                azimuth = get_azimuth(
                    (unitPos['Latitude'], unitPos['Longitude']),
                    (v.dLatitude, v.dLongitude))
                # print('unit: ' + unit.strName, 'ship: ' + v.strName, 'azimuth: %f' % azimuth, 'currentHeading: %f' % unit.fCurrentHeading)
                if disKilo <= v.fAirRangeMax * 1.852 and abs(
                        azimuth - unit.fCurrentHeading) < 90:  # 海里转公里需要乘以1.852
                    # 计算撤退点 TODO
                    ...
                    retreatPos = get_end_point(
                        (unitPos['Latitude'], unitPos['Longitude']), 15,
...
def get_azimuth(geopoint1, geopoint2):
    ...
    azimuth = 180 * math.atan2(
        math.sin(lon2 - lon1),
        math.tan(lat2) * math.cos(lat1) -
        math.sin(lat1) * math.cos(lon2 - lon1)) / PI
...
def get_point_with_point_bearing_distance(lat, lon, bearing, distance):
    ...
    endLatRads = asin((startLatSin * distRatioCosine) +
                      (startLatCos * distRatioSine *
...
def FindBoundingBoxForGivenContacts(contacts, defaults, padding):
    #  Variables
    coordinates = [
        btBas.MakeLatLong(defaults['AI-AO-1']['latitude'],
                          defaults['AI-AO-1']['longitude']),
        btBas.MakeLatLong(defaults['AI-AO-2']['latitude'],
                          defaults['AI-AO-2']['longitude']),
        btBas.MakeLatLong(defaults['AI-AO-3']['latitude'],
                          defaults['AI-AO-3']['longitude']),
        btBas.MakeLatLong(defaults['AI-AO-4']['latitude'],
                          defaults['AI-AO-4']['longitude'])
    ]
...
def AttackDoctrineCreateAirMissionAction(scenario):
    ...
    defaults = {
        ...
        for k, v in side.referencepnts.items() if v.strName in defaultRef
    }
    airContacts_dic = {
        k: v
        for k, v in side.contacts.items() if v.m_ContactType == 0
    }  # 探测到的敌方飞机
    airs_dic = side.aircrafts  # 本方所有飞机
    patrolmssn = [
        v for _, v in side.patrolmssns.items() if v.strName == 'a1_aaw_miss_1'
    ]
    if len(patrolmssn) == 1 or len(airs_dic) == 0 or len(airContacts_dic) == 0:
        return False

    hostileContactBoundingBox = FindBoundingBoxForGivenContacts(
        airContacts_dic, defaults, 3)
    zone = [
        'a1_aaw_miss_1_rp_1', 'a1_aaw_miss_1_rp_2', 'a1_aaw_miss_1_rp_3',
        'a1_aaw_miss_1_rp_4'
    ]
    side.add_reference_point('蓝方', zone[0],
                             hostileContactBoundingBox[0]['latitude'],
                             hostileContactBoundingBox[0]['longitude'])
    side.add_reference_point('蓝方', zone[1],
                             hostileContactBoundingBox[1]['latitude'],
                             hostileContactBoundingBox[1]['longitude'])
    side.add_reference_point('蓝方', zone[2],
                             hostileContactBoundingBox[2]['latitude'],
                             hostileContactBoundingBox[2]['longitude'])
    side.add_reference_point('蓝方', zone[3],
    ...
    a1_aaw_miss = CPatrolMission('T+1_mode', scenario.mozi_server,
                                 scenario.situation)
    ...
    missionUnits = {
        ...
        if ('Top Hatter' in v.strName) or ('Black Ace' in v.strName)
    }
...
def AttackDoctrineCreateAEWMissionAction(scenario):
    ...
    airs_dic = side.aircrafts  # 本方所有飞机
    supportmssn = [
        v for _, v in side.supportmssns.items()
        if v.strName == 'a1_aaew_miss_1'
    ]
    if len(supportmssn) == 1 or len(linkedMissionPoints) == 0 or len(
            airs_dic) == 0 or len(airContacts_dic) == 0:
        return False

    linkedMissionCenterPoint = MidPointCoordinate(
        linkedMissionPoints['a1_aaw_miss_1_rp_1']['latitude'],
    ...
    patrolBoundingBox = FindBoundingBoxForGivenLocations([
        MakeLatLong(linkedMissionCenterPoint['latitude'],
                    linkedMissionCenterPoint['longitude'])
    ], 1.0)
    zone = [
        'a1_aaew_miss_1_rp_1', 'a1_aaew_miss_1_rp_2', 'a1_aaew_miss_1_rp_3',
        'a1_aaew_miss_1_rp_4'
    ]
    side.add_reference_point('蓝方', zone[0], patrolBoundingBox[0]['latitude'],
                             patrolBoundingBox[0]['longitude'])
    side.add_reference_point('蓝方', zone[1], patrolBoundingBox[1]['latitude'],
                             patrolBoundingBox[1]['longitude'])
    side.add_reference_point('蓝方', zone[2], patrolBoundingBox[2]['latitude'],
                             patrolBoundingBox[2]['longitude'])
    side.add_reference_point('蓝方', zone[3], patrolBoundingBox[3]['latitude'],
                             patrolBoundingBox[3]['longitude'])

    side.add_mission_support('a1_aaew_miss_1', zone)  # 支援任务
    a1_aaew_miss = CSupportMission('T+1_mode', scenario.mozi_server,
                                   scenario.situation)
    ...
    missionUnits = {
        k: v
        for k, v in airs_dic.items() if ('Golden Hawk' in v.strName) and (
            int(re.sub('\D', '', v.strName)) <= 2)
    }
...
def AttackDoctrineUpdateAEWAirMissionAction(scenario):
    ...
    if len(supportmssn) == 0 or len(linkedMissionPoints) == 0 or len(
            airs_dic) == 0:
        return False
    ...
    set_str_1 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
        '蓝方', zone[0], patrolBoundingBox[0]['latitude'],
        patrolBoundingBox[0]['longitude'])
    scenario.mozi_server.send_and_recv(set_str_1)
    set_str_2 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
        '蓝方', zone[1], patrolBoundingBox[1]['latitude'],
        patrolBoundingBox[1]['longitude'])
    scenario.mozi_server.send_and_recv(set_str_2)
    set_str_3 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
        '蓝方', zone[2], patrolBoundingBox[2]['latitude'],
        patrolBoundingBox[2]['longitude'])
    scenario.mozi_server.send_and_recv(set_str_3)
    set_str_4 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
        '蓝方', zone[3], patrolBoundingBox[3]['latitude'],
        patrolBoundingBox[3]['longitude'])
    scenario.mozi_server.send_and_recv(set_str_4)
...
def AttackDoctrineCreateStealthAirMissionAction(scenario):
    ...
    patrolmssn = [
        v for _, v in side.patrolmssns.items() if v.strName == 'a1_saaw_miss_1'
    ]
    if len(patrolmssn) == 1 or len(linkedMission) == 0 or len(
            airs_dic) == 0 or len(airContacts_dic) == 0:
        return False
    ...
    a1_saaw_miss = CPatrolMission('T+1_mode', scenario.mozi_server,
                                  scenario.situation)
    ...
    missionUnits = {
        k: v
        for k, v in airs_dic.items() if ('Blackbird' in v.strName) and (
            9 <= int(re.sub('\D', '', v.strName)) <= 12)
    }
...
def SupportAEWDoctrineCreateMissionAction(scenario):
    ...
    supportmssn = [
        v for _, v in side.supportmssns.items() if v.strName == 'aew_sup_miss'
    ]
    if len(supportmssn) == 1 or len(airs_dic) == 0 or len(stennis) == 0:
        return False

    patrolBoundingBox = FindBoundingBoxForGivenLocations(
        [MakeLatLong(stennis[0].dLatitude, stennis[0].dLongitude)], 1)
    zone = [
        'aew_sup_miss_rp_1', 'aew_sup_miss_rp_2', 'aew_sup_miss_rp_3',
        'aew_sup_miss_rp_4'
    ]
    side.add_reference_point('蓝方', zone[0], patrolBoundingBox[0]['latitude'],
                             patrolBoundingBox[0]['longitude'])
    ...
    aew_sup_miss = CSupportMission('T+1_mode', scenario.mozi_server,
                                   scenario.situation)
...
def DefendDoctrineCreateAirMissionAction(scenario):
    ...
    stennis = [v for _, v in ships.items() if 'Stennis' in v.strName]
    patrolmssn = [
        v for _, v in side.patrolmssns.items() if v.strName == 'a1_aaw_d_miss'
    ]
    if len(patrolmssn) == 1 or len(airs_dic) == 0 or len(stennis) == 0:
        return False

    patrolBoundingBox = FindBoundingBoxForGivenLocations(
        [MakeLatLong(stennis[0].dLatitude, stennis[0].dLongitude)], 3)
    zone = [
        'a1_aaw_d_miss_rp_1', 'a1_aaw_d_miss_rp_2', 'a1_aaw_d_miss_rp_3',
        'a1_aaw_d_miss_rp_4'
    ]
    side.add_reference_point('蓝方', zone[0], patrolBoundingBox[0]['latitude'],
                             patrolBoundingBox[0]['longitude'])
    ...
    aaw_d_miss = CPatrolMission('T+1_mode', scenario.mozi_server,
                                scenario.situation)
    ...
    missionUnits = {
        k: v
        for k, v in airs_dic.items() if ('Vigalante' in v.strName) and (
            int(re.sub('\D', '', v.strName)) in (1, 3, 4))
    }
...
def create_patrol_zone(scenario, aoPoints):
    side = scenario.get_side_by_name('蓝方')
    # 生成大四边形的四个中点
    rp12mid = MidPointCoordinate(aoPoints[0]['latitude'],
                                 aoPoints[0]['longitude'],
                                 aoPoints[1]['latitude'],
                                 aoPoints[1]['longitude'])
    rp13mid = MidPointCoordinate(aoPoints[0]['latitude'],
                                 aoPoints[0]['longitude'],
                                 aoPoints[2]['latitude'],
                                 aoPoints[2]['longitude'])
    rp14mid = MidPointCoordinate(aoPoints[0]['latitude'],
                                 aoPoints[0]['longitude'],
                                 aoPoints[3]['latitude'],
                                 aoPoints[3]['longitude'])
    rp23mid = MidPointCoordinate(aoPoints[1]['latitude'],
                                 aoPoints[1]['longitude'],
                                 aoPoints[2]['latitude'],
                                 aoPoints[2]['longitude'])
    rp34mid = MidPointCoordinate(aoPoints[2]['latitude'],
                                 aoPoints[2]['longitude'],
                                 aoPoints[3]['latitude'],
                                 aoPoints[3]['longitude'])
    # 如果是rec巡逻任务1，添加4个参考点：rp1，rp2，rp3，rp4
    # 巡逻任务1
    side.add_reference_point('蓝方', 'rp2', rp12mid['latitude'],
                             rp12mid['longitude'])
    side.add_reference_point('蓝方', 'rp3', rp13mid['latitude'],
                             rp13mid['longitude'])
    side.add_reference_point('蓝方', 'rp4', rp14mid['latitude'],
                             rp14mid['longitude'])
    zone0 = ['AI-AO-1', 'rp2', 'rp3', 'rp4']
    # 巡逻任务2
    side.add_reference_point('蓝方', 'rp5', rp12mid['latitude'],
                             rp12mid['longitude'])
    side.add_reference_point('蓝方', 'rp7', rp23mid['latitude'],
                             rp23mid['longitude'])
    side.add_reference_point('蓝方', 'rp8', rp13mid['latitude'],
                             rp13mid['longitude'])
    zone1 = ['rp5', 'AI-AO-2', 'rp7', 'rp8']

    # 巡逻任务3
    side.add_reference_point('蓝方', 'rp9', rp13mid['latitude'],
                             rp13mid['longitude'])
    side.add_reference_point('蓝方', 'rp10', rp23mid['latitude'],
                             rp23mid['longitude'])
    side.add_reference_point('蓝方', 'rp12', rp34mid['latitude'],
                             rp34mid['longitude'])
    zone2 = ['rp9', 'rp10', 'AI-AO-3', 'rp12']

    # 巡逻任务4
    side.add_reference_point('蓝方', 'rp13', rp14mid['latitude'],
                             rp14mid['longitude'])
    side.add_reference_point('蓝方', 'rp14', rp13mid['latitude'],
                             rp13mid['longitude'])
    side.add_reference_point('蓝方', 'rp15', rp34mid['latitude'],
                             rp34mid['longitude'])
...
def ReconDoctrineCreateMissionAction(scenario):
    ...
    recon_missions = [
        ...
        if 'rec_miss' in mission.strName
    ]

    if len(recon_missions) >= 4 or len(recon_airs) == 0 or len(
            side.contacts) >= 10 or len(defaults) != 4:
        return False
    ...
    for zone in [zone0, zone1, zone2, zone3]:
        ...
        patrolmssn = CPatrolMission('T+1_mode', scenario.mozi_server,
                                    scenario.situation)
        ...
        recon_air = {
            k: v
            for k, v in recon_airs.items()
            if int(re.sub('\D', '', v.strName)) in (i * 2 - 1, i * 2)
        }
...
def UpdateAIAreaOfOperations(scenario):
    ...
    defaults = {
        ...
        for k, v in side.referencepnts.items() if v.strName in zone
    }
    ...
    if len(defaults) < 4:
        ...
        for i in range(len(patrolBoundingBox)):
            cmd = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
                '蓝方', 'AI-AO-' + str(i + 1), patrolBoundingBox[i]['latitude'],
...
def SupportTankerDoctrineCreateMissionAction(scenario):
    ...
    patrolBoundingBox = FindBoundingBoxForGivenLocations(
        [MakeLatLong(stennis[0].dLatitude, stennis[0].dLongitude)], 0.5)
    zone = [
        'tan_sup_miss_rp_1', 'tan_sup_miss_rp_2', 'tan_sup_miss_rp_3',
        'tan_sup_miss_rp_4'
    ]
    side.add_reference_point('蓝方', zone[0], patrolBoundingBox[0]['latitude'],
                             patrolBoundingBox[0]['longitude'])
    ...
    tan_sup_miss = CSupportMission('T+1_mode', scenario.mozi_server,
                                   scenario.situation)
...
def AttackDoctrineCreateAntiSurfaceShipMissionAction(scenario):
    ...
    strikemssn = [
        v for _, v in side.strikemssns.items() if v.strName == 'asuw_miss'
    ]
    if len(strikemssn) == 1 or len(airs_dic) == 0 or len(targets) == 0:
        return False

    side.add_mission_strike('asuw_miss', 2)
    asuw_miss = CStrikeMission('T+1_mode', scenario.mozi_server,
                               scenario.situation)
    ...
    missionUnits = {
        k: v
        for k, v in airs_dic.items()
        if ('Warhawk' in v.strName) or (('Vigalante' in v.strName) and (
            int(re.sub('\D', '', v.strName)) not in (1, 3, 4)))
    }
